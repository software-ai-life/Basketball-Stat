export default function PlayerStats({ teams, stats, scores }) {
  ...
  const getPlayersWithStats = (teamKey) => (
    Object.entries(stats)
      .filter(([_, s]) => s.team === teamKey)
      .map(([id, s]) => ({
        id,
        name: s.name,
        stats: s,
        isOnCourt: teams[teamKey].players.some(p => p.id === id)
      }))
      .sort((a, b) => calculateTotalPoints(b.stats) - calculateTotalPoints(a.stats))
  )

  const renderTeamSection = (teamKey, accentClass) => {
    const players = getPlayersWithStats(teamKey)
      ...
      <div className="mb-8">
        <div className="mb-6">
          <div className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-full border border-gray-200">
            <span className={`text-base ${accentClass}`}>{teamKey === 'teamA' ? '★' : '●'}</span>
            <span className="text-sm font-medium text-dark">{teams[teamKey].name} 完整統計</span>
          </div>
        </div>

        <div className="space-y-3">
          {players.map(({ id, name, stats: playerStats, isOnCourt }) => {
            ...
            return (
              <div key={id} className={`card p-5 ${!isOnCourt ? 'opacity-70 border-l-4 border-l-gray-300' : ''}`}>
                <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
                  ...
                  <div className={`text-3xl font-serif font-bold tabular-nums ${accentClass}`}>
                    {totalPoints} 分
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3 text-xs mb-4">
                  <div className="text-center">
                    ...
                    <div className="font-mono tabular-nums text-dark text-base font-medium">{twoP}/{twoPA}</div>
                    {twoPA > 0 && <div className="text-xs text-accent mt-1">{Math.round((twoP / twoPA) * 100)}%</div>}
                  </div>
                  <div className="text-center">
                    <div className="text-dark/40 mb-1 uppercase tracking-wider">3分</div>
                    <div className="font-mono tabular-nums text-dark text-base font-medium">{threeP}/{threePA}</div>
                    {threePA > 0 && <div className="text-xs text-accent mt-1">{Math.round((threeP / threePA) * 100)}%</div>}
                  </div>
                  <div className="text-center">
                    <div className="text-dark/40 mb-1 uppercase tracking-wider">抄截</div>
                    <div className="font-mono tabular-nums text-dark text-base font-medium">{playerStats?.steals || 0}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-dark/40 mb-1 uppercase tracking-wider">助攻</div>
                    <div className="font-mono tabular-nums text-dark text-base font-medium">{playerStats?.assists || 0}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3 text-xs">
                  <div className="text-center">
                    ...
                    <div className="font-mono tabular-nums text-dark text-base font-medium">{playerStats?.offensiveRebounds || 0}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-dark/40 mb-1 uppercase tracking-wider">防守板</div>
                    <div className="font-mono tabular-nums text-dark text-base font-medium">{playerStats?.defensiveRebounds || 0}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-dark/40 mb-1 uppercase tracking-wider">阻攻</div>
                    <div className="font-mono tabular-nums text-dark text-base font-medium">{playerStats?.blocks || 0}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-dark/40 mb-1 uppercase tracking-wider">失誤</div>
                    <div className="font-mono tabular-nums text-dark text-base font-medium">{playerStats?.turnovers || 0}</div>
                  </div>
                </div>
              </div>
            )
  ...
  return (
    <div className="px-6 pt-6 pb-6">
      {renderTeamSection('teamA', 'text-accent')}
      {renderTeamSection('teamB', 'text-dark')}

      <div className="card p-8 bg-gradient-to-br from-white to-gray-50">
        <div className="grid grid-cols-2 gap-6 text-center">
          <div>
            <div className="text-xs text-dark/40 mb-2 uppercase tracking-wider">{teams.teamA.name}</div>
            <div className="text-5xl font-serif font-bold tabular-nums text-accent">{scores.teamA}</div>
            <div className="text-sm text-dark/60 font-medium mt-2">總分</div>
          </div>
          <div>
            <div className="text-xs text-dark/40 mb-2 uppercase tracking-wider">{teams.teamB.name}</div>
            <div className="text-5xl font-serif font-bold tabular-nums text-dark">{scores.teamB}</div>
            <div className="text-sm text-dark/60 font-medium mt-2">總分</div>
          </div>
        </div>
      </div>
